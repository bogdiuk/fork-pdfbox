...
final class FileSystemFontProvider extends FontProvider
{
     ...
     * After implementing {@link LoadOnlyHeaders}, parsing font headers is faster than checksumming.
     */
    ...
    private FSFontInfo createFSIgnored(File file, FontFormat format, String postScriptName)
    {
        ...
        try
        {
            hash = SKIP_CHECKSUMS ? CHECKSUM_PLACEHOLDER : computeHash(file);
        }
        catch (IOException ex)
        {
            hash = "";
    ...
    {
        try (TrueTypeCollection ttc = new TrueTypeCollection(ttcFile, true))
        {
            String hash = SKIP_CHECKSUMS ? CHECKSUM_PLACEHOLDER : computeHash(ttcFile);
            ttc.processAllFontHeaders(ttf -> addTrueTypeFontImpl(ttf, ttcFile, hash));
        }
        catch (IOException e)
        {
    ...
    private void addTrueTypeFont(File ttfFile) throws IOException
    {
        ...
        try
        {
            ...
            parser.setLoadOnlyHeaders(headers);
            IOUtils.closeQuietly(parser.parse(new RandomAccessReadBufferedFile(ttfFile)));
            addTrueTypeFontImpl(headers, ttfFile,
                    SKIP_CHECKSUMS ? CHECKSUM_PLACEHOLDER : computeHash(ttfFile));
        }
        catch (IOException e)
        {
     ...
     * Adds an OTF or TTF font to the file cache. To reduce memory, the parsed font is not cached.
     */
    private void addTrueTypeFontImpl(LoadOnlyHeaders ttf, File file, String fileHash) throws IOException
    {
        final IOException exception = ttf.getException();
        if (exception == null)
        {
            ...
            else if (name != null)
            {
                ...
                {
                    byte[] gcid = ttf.getNonOtfTableGCID142();
                    if (gcid != null)
                    {
                        // Apple's AAT fonts have a "gcid" table with CID info
                        String reg = new String(gcid, 10, 64, StandardCharsets.US_ASCII);
                        String registryName = reg.substring(0, reg.indexOf('\0'));
                        String ord = new String(gcid, 76, 64, StandardCharsets.US_ASCII);
                        String orderName = ord.substring(0, ord.indexOf('\0'));
                        int supplementVersion = gcid[140] << 8 & (gcid[141] & 0xFF);
        ...
        else
        {
            ...
            LOG.warn("Could not load font file: " + file, exception);
        }
    }
    ...
    private void addType1Font(File pfbFile) throws IOException
    {
        try (InputStream input = new FileInputStream(pfbFile))
        {
            ...
            String hash = SKIP_CHECKSUMS ? CHECKSUM_PLACEHOLDER : computeHash(pfbFile);
